...
def rephrase_line(line: str) -> str:
    ...
    return ''.join([rephrase_word(word) for word in re.findall(r'\w+|[^\w\s]', line.strip())])

def rephrase_text(text: str) -> str:

    return ''.join([rephrase_line(line) for line in text.splitlines(keepends=True)])
...
def rephrase_file(input_file: str, output_file: str) -> None:
    ...
    with open(input_file) as f:
        ...
        with open(output_file, 'w') as out:
            ...
            out.write(rephrase_text(f.read()))
