...
def download_media(url_tag: Tuple[str, str], max_sizes: Dict[str, int]) -> None:

    tag, url = url_tag

    if url and url.startswith('http') and not BS(str(tag), 'html.parser').has_attr('data-no-download'):

        download_file(url, generate_file_name(url, tag, BS(str(tag), 'html.parser').get('alt')), max_sizes.get(tag))

def download_document(url: str) -> None:

    if url and url.startswith('http'):

        download_file(url, generate_file_name(url, '', ''), 0)
...
try:
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for url_tag in media_tags:

            executor.submit(download_media, url_tag, max_sizes)

        for url in document_links:

            executor.submit(download_document, url)
